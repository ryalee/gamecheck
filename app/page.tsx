...
export default function Page() {
  ...
  return (
    <main>
      <Header showActions={gc.state === "done"} onReset={gc.reset} />

      <div className="content">
        {gc.state === "idle" && <LandingView onAnalyze={gc.analyze} />}

        {gc.state === "scanning" && (
          <ScanningView scanStep={gc.scanStep} scanSteps={gc.scanSteps} />
        )}

        {gc.state === "loading" && <LoadingView />}
        ...
        {gc.state === "done" && gc.specs && (
          <ResultsView
            ...
            searchedGames={gc.searchedGames}
            searchLoading={gc.searchLoading}
            searchQuery={gc.searchQuery}
            filter={gc.filter}
